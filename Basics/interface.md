...
- **如果说,USB口是一个接口,任何想要与它连接的插头都必须按照它的约定去实现,比如它的大小,形状,材料等等**;

- **接口是一个定义好的模型, 模型中指定了需用实现的字段与字段的类型;类似于后端给我们前端的接口,只有按照它要求的字段/类型去请求.双方才能正常通信**
...
function hello(person: Person): string{
    return `你好啊,我是${person.name},年龄是${person.age},性别是${person.man?'♂':'♀'},我是个${person.professional}!`
}

const sweetou = { professional:'快乐的码农', name: 'sweetou', age:50, man: true};

let swt: string = hello(sweetou); // 你好啊,我是sweetou,年龄是50,性别是♂,我是个快乐的码农!
...
在上面的例子里，`SelectableControl`包含了`Control`的所有成员，包括私有成员state。 因为 `state`是私有成员，所以只能够是`Control`的子类们才能实现`SelectableControl`接口。 因为只有 `Control`的子类才能够拥有一个声明于`Control`的私有成员`state`，这对私有成员的兼容性是必需的。
